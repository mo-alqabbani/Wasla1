...
// User Routes functions (optimized)
function loadUserRoutes() {
    ...
    if (!cachedRoutes || container.children.length === 0) {
        container.innerHTML = '';
    }
...
function loadCreditPackages() {
    ...
    if (cachedPackages && container.children.length > 0) {
        return; // Don't recreate if already loaded
    }
